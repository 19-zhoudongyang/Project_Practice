...
- [Nginx](#nginx)
  ...
  - [具体应用](#具体应用)
    ...
    - [负载均衡](#负载均衡)
      - [配置负载均衡](#配置负载均衡)
        - [因为Session是存放在服务器端的，所以使用负载均衡不能用Session存储登录状态；解决方案：](#因为session是存放在服务器端的所以使用负载均衡不能用session存储登录状态解决方案)
          - [1.使用redis或者cookie存储登录状态；](#1使用redis或者cookie存储登录状态)
          - [2.或者让服务器之间共享Session;](#2或者让服务器之间共享session)
          - [3.改变负载均衡的默认策略，使用ip\_hash策略，根据客户端的ip计算hash值，使得同一个客户端访问到的服务器是同一个，可以正常获取session](#3改变负载均衡的默认策略使用ip_hash策略根据客户端的ip计算hash值使得同一个客户端访问到的服务器是同一个可以正常获取session)
- [前后端分离开发](#前后端分离开发)
  - [问题说明](#问题说明-2)
  - [前后端分离开发](#前后端分离开发-1)
    - [介绍](#介绍-2)
...
#### 配置负载均衡
>>>>![1](pic/192.png)
##### 因为Session是存放在服务器端的，所以使用负载均衡不能用Session存储登录状态；解决方案：
###### 1.使用redis或者cookie存储登录状态；
###### 2.或者让服务器之间共享Session;
###### 3.改变负载均衡的默认策略，使用ip_hash策略，根据客户端的ip计算hash值，使得同一个客户端访问到的服务器是同一个，可以正常获取session
>>>>![1](pic/193.png)
>>>>![1](pic/194.png)
---
---
